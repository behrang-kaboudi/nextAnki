...
function StatusPill({
  ...
  status: "dirty" | "verified" | "unverified";
}) {
  const classes =
    status === "dirty"
      ? "bg-amber-600"
  ...
  return (
    <span
      className={[
        "inline-block size-4 rounded-full ring-2 ring-card",
        classes,
      ].join(" ")}
    >
...
function defaultUsageForDraft(row: Pick<PictureWordRow, "usage" | "canBePersonal" | "ipaVerified">): PictureWordRow["usage"] {
  if (row.canBePersonal) return "person";
  if (!row.ipaVerified) return "Job";
  return row.usage;
}
...
export function PictureWordsClient() {
  ...
  const [activeField, setActiveField] = useState<
    | { kind: "draft"; id: number; field: "fa" | "ipa_fa" | "phinglish" | "en" }
    | { kind: "search" }
    | { kind: "import" }
    | null
  >(null);
        ...
        for (const row of data.rows) {
          if (!next[row.id]) {
            next[row.id] = {
              ...
              en: row.en,
              usage: defaultUsageForDraft(row),
    ...
    async (id: number) => {
      ...
      const draft =
        drafts[id] ?? {
          ...
          en: previous.en,
          usage: defaultUsageForDraft(previous),
          canBePersonal: previous.canBePersonal,
        };

      if (draft.usage === "notSet") {
        setTableError("usage must not be notSet. Please select a usage first.");
        setTableNotice(null);
        return;
      }

      const effectiveUsage = draft.canBePersonal ? "person" : draft.usage;
    ...
    try {
      ...
      setDrafts((prev) => ({
        ...
        [data.row!.id]: {
          ...
          en: data.row!.en,
          usage: defaultUsageForDraft(data.row!),
  ...
  const dirtyIds = useMemo(() => {
    ...
    for (const row of rows) {
      if (hasRowChanges(row)) ids.push(row.id);
    }
    return ids;
  }, [rows, hasRowChanges]);
  ...
  const saveAllDirty = useCallback(async () => {
    for (const id of dirtyIds) {
      // sequential to avoid hammering API + race conditions
      await onVerify(id);
    }
  }, [dirtyIds, onVerify]);
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (!isSave) return;
      e.preventDefault();
      if (savingId || tableLoading || importLoading) return;

      if (activeField?.kind === "draft") {
        void onVerify(activeField.id);
        return;
      }
      if (dirtyIds.length) void saveAllDirty();
    };
      ...
      if (activeField?.kind === "draft") {
        ...
        setDrafts((prev) => ({
          ...
          [activeField.id]: {
            ...
            en:
              activeField.field === "en"
                ? nextValue
                : prev[activeField.id]?.en ?? "",
  ...
  return (
    <div className="grid gap-4">
      {isImportOpen ? (
        <div
          className="fixed inset-0 z-50 grid place-items-center overflow-auto bg-black/40 p-4"
          role="dialog"
          aria-modal="true"
          onMouseDown={(e) => {
        ...
        >
          <div className="w-[min(80vw,64rem)]">
            <div className="mb-3 flex items-center justify-end">
              <button
                type="button"
                onClick={() => setIsImportOpen(false)}
                className="rounded-xl border border-card bg-background px-3 py-2 text-sm text-foreground"
              >
                Close
              </button>
            </div>
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-xl bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-[var(--primary-foreground)] shadow-elevated transition hover:opacity-95"
            >
              Import
            </button>
            {dirtyIds.length ? (
              <button
                ...
                disabled={tableLoading || importLoading || !!savingId}
                className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-2 text-sm font-semibold text-amber-800 shadow-elevated transition hover:bg-amber-500/15 disabled:opacity-60"
              >
            ...
            <label className="inline-flex select-none items-center gap-2 rounded-xl border border-card bg-background px-3 py-2 text-sm text-foreground">
              ...
              />
              Unverified ({rows.filter((r) => !r.ipaVerified).length})
            </label>
            <label className="inline-flex select-none items-center gap-2 rounded-xl border border-card bg-background px-3 py-2 text-sm text-foreground">
              <input
                type="checkbox"
              ...
              />
              notSet ({rows.filter((r) => r.usage === "notSet").length})
            </label>
            <label className="inline-flex select-none items-center gap-2 rounded-xl border border-card bg-background px-3 py-2 text-sm text-foreground">
              <input
                type="checkbox"
              ...
              <table className="w-full table-fixed border-collapse">
                <tbody>
                  <tr className="border-t border-card text-[1.05rem]">
                    ...
                    <td className="break-words px-4 py-3 text-foreground">
                      <input
                        value={drafts[lastChangedRow.id]?.en ?? lastChangedRow.en}
                        onChange={(e) =>
                          setDrafts((prev) => ({
                            ...prev,
                            [lastChangedRow.id]: {
                              fa: prev[lastChangedRow.id]?.fa ?? lastChangedRow.fa,
                              ipa_fa:
                                prev[lastChangedRow.id]?.ipa_fa ??
                                lastChangedRow.ipa_fa,
                              phinglish:
                        ...
                        disabled={!rows.some((row) => row.id === lastChangedRow.id)}
                        onFocus={(e) => {
                          lastFocusedInputRef.current = e.currentTarget;
                          setActiveField({
                            kind: "draft",
                        ...
                        className="w-full rounded-lg border border-card bg-background px-2 py-1 text-[1.1rem] text-foreground disabled:opacity-60"
                        aria-label={`en ${lastChangedRow.id}`}
                      />
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex justify-center">
                        <input
                          ...
                          aria-label={`Personal ${lastChangedRow.id}`}
                        />
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <select
                        value={drafts[lastChangedRow.id]?.usage ?? lastChangedRow.usage}
                        onChange={(e) => {
                          const value = e.target.value as PictureWordRow["usage"];
                          setDrafts((prev) => ({
                            ...prev,
                            [lastChangedRow.id]: {
                              fa: prev[lastChangedRow.id]?.fa ?? lastChangedRow.fa,
                              ...
                              phinglish:
                                prev[lastChangedRow.id]?.phinglish ??
                                lastChangedRow.phinglish,
                              en: prev[lastChangedRow.id]?.en ?? lastChangedRow.en,
                              usage: value,
                              canBePersonal:
                                prev[lastChangedRow.id]?.canBePersonal ??
                                lastChangedRow.canBePersonal,
                        ...
                        disabled={!rows.some((row) => row.id === lastChangedRow.id)}
                        className="w-full rounded-lg border border-card bg-background px-2 py-1 text-sm text-foreground disabled:opacity-60"
                        aria-label={`usage ${lastChangedRow.id}`}
                      >
            ...
            <table className="w-full table-auto border-separate border-spacing-0">
              <thead className="sticky top-0 z-10 bg-background/95 backdrop-blur">
                <tr className="text-left text-xs font-semibold uppercase tracking-wide text-muted">
                  ...
                  <th className="hidden whitespace-nowrap border-b border-r border-card px-3 py-2 lg:table-cell">
                    ipa
                  </th>
                  <th className="hidden whitespace-nowrap border-b border-r border-card px-3 py-2 lg:table-cell">
                    phinglish
                  ...
                  {showUnverifiedOnly ? (
                    <th className="w-28 whitespace-nowrap border-b border-r border-card px-3 py-2">
                      Verify
                    </th>
                  ) : null}
                  <th className="w-[14%] whitespace-nowrap border-b border-r border-card px-3 py-2">
                    en
                  </th>
                  <th
                    className="w-[56px] whitespace-nowrap border-b border-r border-card px-3 py-2 text-center"
                    title="Personal: whether this word can be used as a personal word"
                  >
                    Personal
                  </th>
                  <th
                    className="w-[140px] whitespace-nowrap border-b border-r border-card px-3 py-2"
                    title="Usage: how this entry is used (enum)"
                  >
                    Usage
                  </th>
                  {!showUnverifiedOnly ? (
                  ...
                  <th className="w-14 whitespace-nowrap border-b border-card px-2 py-2 text-center">
              ...
              <tbody>
                {sorted.map((row) => (
                  <tr
                    key={row.id}
                    className="text-[1.05rem] odd:bg-background/40 hover:bg-card/60"
                  >
                    <td className="border-b border-r border-card px-3 py-2 align-middle text-center">
                    ...
                    <td className="break-words border-b border-r border-card px-3 py-2 text-foreground">
                      <input
                        ...
                        onChange={(e) =>
                          setDrafts((prev) => ({
                            ...
                            [row.id]: {
                              ...
                              en: prev[row.id]?.en ?? row.en,
                              usage: prev[row.id]?.usage ?? row.usage,
                              canBePersonal:
                    ...
                    <td className="hidden border-b border-r border-card px-3 py-2 lg:table-cell">
                      <input
                    ...
                    {showUnverifiedOnly ? (
                      <td className="border-b border-r border-card px-3 py-2">
                        <button
                          type="button"
                        ...
                        onChange={(e) =>
                          setDrafts((prev) => ({
                            ...
                            [row.id]: {
                              fa: prev[row.id]?.fa ?? row.fa,
                              ipa_fa: prev[row.id]?.ipa_fa ?? row.ipa_fa,
                              phinglish: prev[row.id]?.phinglish ?? row.phinglish,
                              en: e.target.value,
                              usage: prev[row.id]?.usage ?? row.usage,
                              canBePersonal:
                                prev[row.id]?.canBePersonal ?? row.canBePersonal,
                            },
                          }))
                        }
                        onFocus={(e) => {
                          lastFocusedInputRef.current = e.currentTarget;
                          setActiveField({ kind: "draft", id: row.id, field: "en" });
                        }}
                        className="w-full rounded-lg border border-card bg-background px-2 py-1 text-[1.25rem] text-foreground"
                          ...
                          onChange={(e) =>
                            setDrafts((prev) => ({
                              ...
                              [row.id]: {
                                ...
                                usage: e.target.checked
                                  ? "person"
                                  : prev[row.id]?.usage ?? row.usage,
                                canBePersonal: e.target.checked,
                              },
                            }))
                          }
                          className="h-5 w-5 accent-[var(--primary)]"
                          aria-label={`Personal ${row.id}`}
                        />
                      </div>
                    </td>
                    <td className="border-b border-r border-card px-3 py-2">
                      <select
                        value={drafts[row.id]?.usage ?? row.usage}
                        onChange={(e) => {
                          const value = e.target.value as PictureWordRow["usage"];
                          setDrafts((prev) => ({
                            ...prev,
                        ...
                        className="w-[140px] rounded-lg border border-card bg-background px-2 py-1 text-sm text-foreground"
                        aria-label={`usage ${row.id}`}
                      >
                        {USAGE_VALUES.map((value) => (
                          <option key={value} value={value}>
                            {value}
                    ...
                    <td className="border-b border-card px-2 py-2 text-center">
                      <button
                        type="button"
