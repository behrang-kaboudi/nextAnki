...
export default function PersianWordMeaningIpaPhase2() {
  ...
  const openPrompt = useCallback(async () => {
    ...
    try {
      ...
      setPrompt(files.join("\n\n")); setData(JSON.stringify(missing.items ?? [], null, 2)); setRemainingCount(typeof missing.totalMissing === "number" ? missing.totalMissing : null);
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setLoading(false); }
  }, [limit]);
