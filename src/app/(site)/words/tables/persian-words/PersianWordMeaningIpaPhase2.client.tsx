...
type InputRow = { id: number; canonical_text: string; dbMeaning: string | null; dbNormalized: string | null; inputMeaning: string; saving: boolean; saved: boolean; error: string | null };
...
export default function PersianWordMeaningIpaPhase2({ initialMissingCount }: { initialMissingCount: number }) {
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/words/persian-words/meaning-fa-ipa/missing?limit=${encodeURIComponent(limit)}`);
  ...
  const openPrompt = useCallback(async () => {
    ...
    try {
      const [files, missing] = await Promise.all([
        ...
        fetch(`/api/words/persian-words/meaning-fa-ipa/missing?limit=${encodeURIComponent(limit)}`).then(async (res) => { const json = (await res.json()) as { ok?: boolean; items?: unknown; totalMissing?: number; error?: string }; if (!res.ok || !json.ok) throw new Error(json.error || "Could not load missing rows."); return json; }),
      ]);
      setPrompt(files.join("\n\n")); setData(JSON.stringify(missing.items ?? [], null, 2)); setRemainingCount(typeof missing.totalMissing === "number" ? missing.totalMissing : initialMissingCount);
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); } finally { setLoading(false); }
  }, [initialMissingCount, limit]);
  ...
  const prepareApply = useCallback(async () => {
    ...
    try {
      ...
      const json = (await res.json()) as { ok?: boolean; items?: Array<{ id: number; canonical_text: string; meaning_fa_IPA: string | null; meaning_fa_IPA_normalize: string | null }>; error?: string };
      ...
      setRows(pairs.map((pair) => { const item = db.get(pair.id); if (!item) throw new Error(`PersianWord ${pair.id} was not found.`); return { id: item.id, canonical_text: item.canonical_text, dbMeaning: item.meaning_fa_IPA, dbNormalized: item.meaning_fa_IPA_normalize, inputMeaning: pair.meaning_fa_IPA, saving: false, saved: false, error: null }; }));
      setConfirmOpen(true);
    } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
  }, [response]);
  ...
  const apply = useCallback(async (selected: InputRow[]) => {
    ...
    const json = (await res.json()) as { ok?: boolean; total?: number; updated?: number; results?: Array<{ ok: boolean; id: number; meaning_fa_IPA?: string | null; meaning_fa_IPA_normalize?: string | null; error?: string; duplicateConflict?: DuplicateConflict }>; error?: string };
    ...
    setRows((current) => current.map((row) => { const result = results.get(row.id); return !result ? row : result.ok ? { ...row, dbMeaning: result.meaning_fa_IPA ?? row.inputMeaning, dbNormalized: result.meaning_fa_IPA_normalize ?? null, saving: false, saved: true, error: null } : { ...row, saving: false, error: result.error ?? "Update failed." }; }));
    router.refresh();
  }, [router]);
  ...
  const insertCharacter = (character: string) => {
    ...
    setRows((current) => current.map((row) => row.id === id ? { ...row, inputMeaning: next, saved: false } : row));
