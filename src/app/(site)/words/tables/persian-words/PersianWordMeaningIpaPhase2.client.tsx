  ...
  const openPrompt = useCallback(async () => {
    ...
    try {
      ...
      const [files, missing] = await Promise.all([
        Promise.all(paths.map(async (path) => { const res = await fetch(`/api/ai/prompt-file?path=${encodeURIComponent(path)}`); const json = (await res.json()) as { text?: string; error?: string }; if (!res.ok || !json.text) throw new Error(json.error || "Could not load prompt."); return json.text; })),
