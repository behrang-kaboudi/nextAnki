...
type Report = { total: number; updated: number; failed: number };
...
export default function PersianWordMeaningIpaPhase2() {
  ...
  const apply = useCallback(async (selected: InputRow[]) => {
    ...
    const json = (await res.json()) as { ok?: boolean; total?: number; updated?: number; results?: Array<{ ok: boolean; id: number; meaning_fa_IPA?: string | null; meaning_fa_IPA_normalize?: string | null; error?: string }>; error?: string };
    ...
    setReport({ total: json.total ?? selected.length, updated: json.updated ?? 0, failed });
