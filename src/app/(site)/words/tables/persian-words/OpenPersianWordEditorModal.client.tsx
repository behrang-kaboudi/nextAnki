        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6">
          ...
          {item ? <div className="grid gap-4 lg:grid-cols-2">
            ...
            <section className="rounded border border-amber-500/30 bg-amber-500/5 p-3 text-sm lg:col-span-2">
              ...
              <p className="mt-1 text-xs opacity-80">Saving text, IPA, or audio here updates the sync timestamp of every listed WordSense.</p>
              {references.length ? <ul className="mt-2 max-h-36 space-y-1 overflow-auto font-mono text-xs">{references.map((reference) => <li key={reference.id}>#{reference.id} — {reference.base_form} <span className="opacity-70">({reference.roles.join(", ")})</span></li>)}</ul> : <p className="mt-2 text-xs opacity-70">No WordSense is currently linked to this PersianWord.</p>}
            </section>
            <div className="flex flex-wrap items-end gap-2 lg:col-span-2"><label className="grid flex-1 gap-1 text-sm">Audio file name <span className="rounded border bg-black/5 px-3 py-2 font-mono text-xs opacity-70">{item.audio_file_name ?? "—"}</span></label><label className="grid flex-1 gap-1 text-sm">Audio source text <span className="rounded border bg-black/5 px-3 py-2 text-xs opacity-70" dir="rtl">{item.audio_source_text ?? "—"}</span></label><PersianWordAudioControls id={id} filename={item.audio_file_name} onFilenameChange={(filename) => setItem((current) => current ? { ...current, audio_file_name: filename, audio_source_text: filename ? current.canonical_text.trim() : null } : current)} /></div>
