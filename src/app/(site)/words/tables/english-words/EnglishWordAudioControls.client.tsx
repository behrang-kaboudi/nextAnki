...
export default function EnglishWordAudioControls({ id, filename: initialFilename }: { id: number; filename: string | null }) {
  ...
  const updateFilename = useCallback((nextFilename: string | null) => { setFilename(nextFilename); router.refresh(); }, [router]);
  ...
  const request = useCallback(async (url: string, init?: RequestInit) => {
    ...
    if (!response.ok || !payload?.ok) throw new Error(payload?.error || "Audio request failed.");
    return payload;
  }, []);
  const uploadRecording = useCallback(async (blob: Blob) => {
    setBusy(true); setError(null); setNotice(null);
    try { const form = new FormData(); form.set("audio", blob, "recording.webm"); const payload = await request(`/api/words/english-words/${id}/audio`, { method: "POST", body: form }); if (!payload.filename) throw new Error("Could not save recording."); updateFilename(payload.filename); setNotice("Recording saved ✓"); }
  ...
  const generate = useCallback(async () => {
    ...
    setBusy(true); setError(null); setNotice(null);
    try { const payload = await request(`/api/words/english-words/${id}/audio/generate`, { method: "POST" }); if (!payload.filename) throw new Error("Could not generate audio."); updateFilename(payload.filename); setNotice("Audio generated ✓"); }
    catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(false); }
  ...
  const deleteAudio = useCallback(async () => {
    ...
    setBusy(true); setError(null); setNotice(null);
    try { await request(`/api/words/english-words/${id}/audio/delete`, { method: "POST" }); updateFilename(null); setNotice("Audio deleted ✓"); }
    catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(false); }
  ...
  return <div className="flex flex-wrap items-center gap-1">
    {filename ? <audio ref={audioRef} preload="none" src={getEnglishWordAudioPublicPath(filename)} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => setPlaying(false)} /> : null}
