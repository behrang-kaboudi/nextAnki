...
import {
  ...
  deleteDeletablePersianWords,
} from "@/lib/persian/unreferencedPersianWordMaintenance.server";

export const runtime = "nodejs";

export async function GET() {
  try {
    return NextResponse.json({ ok: true, count: await countDeletablePersianWords() });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json({
      ok: true,
      result: await deleteDeletablePersianWords({
