...
export async function GET(request: Request) {
  ...
  const [raw, totalUnconfirmed] = await Promise.all([
    prisma.word.findMany({ where, orderBy: { id: "asc" }, take, select: { id: true, meaningId: true, otherMeaningIds: true, english: { select: { base_form: true, phonetic_us: true } }, sentence: { select: { sentence_en: true } } } }),
