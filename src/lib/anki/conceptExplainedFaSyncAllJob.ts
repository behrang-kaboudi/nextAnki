...
export type ConceptExplainedFaSyncAllStatus = {
  jobId: string;
  running: boolean;
  done: boolean;
...
type State = ConceptExplainedFaSyncAllStatus & { _started: boolean };
...
function getState(): State {
  const g = globalThis as unknown as { __conceptExplainedFaSyncAll?: State };
  if (!g.__conceptExplainedFaSyncAll) {
  ...
  return g.__conceptExplainedFaSyncAll;
}

export function getConceptExplainedFaSyncAllStatus(): ConceptExplainedFaSyncAllStatus {
  const s = getState();
...
function indexLatestAudioForConceptExplainedFa(): Map<string, ExistingFileInfo> {
  ...
  const reNew = new RegExp(`^(?<anki>.+?)${sep}concept_explained_fa${sep}(?<ts>\\d{8,})\\.mp3$`);
  const reLegacy = new RegExp(`^(?<anki>.+)_concept_explained_fa_(?<ts>\\d{8,})\\.mp3$`);
...
function generateConceptExplainedFaValue(
  dbValue: string,
  ankiLinkId: string,
...
async function updateNoteField(noteId: number, value: string, anki: ReturnType<typeof createAnkiConnectClient>) {
  const res = await anki.requestDetailed("updateNoteFields", {
    note: { id: noteId, fields: { concept_explained_fa: value } },
  });
...
async function runJob(state: State) {
  ...
  const audioIndex = indexLatestAudioForConceptExplainedFa();
  ...
  for (const batch of chunk(ids, 250)) {
    ...
    for (const n of infoRes.result ?? []) {
      ...
      const value = String(n.fields?.concept_explained_fa?.value ?? "");
  ...
  for (const group of chunk(allIds, 1000)) {
    ...
    for (const r of rows) dbValueByAnkiLinkId.set(r.anki_link_id, r.concept_explained_fa ?? "");
  }
  ...
  await runWithConcurrency(ids, concurrency, async (noteId) => {
    ...
    const newValue = generateConceptExplainedFaValue(dbValue, ankiLinkId, audioIndex);
...
export function startConceptExplainedFaSyncAllIfNeeded(): ConceptExplainedFaSyncAllStatus {
  const state = getState();
  if (state.running) return getConceptExplainedFaSyncAllStatus();
  if (state._started && !state.done) return getConceptExplainedFaSyncAllStatus();

  state.jobId = `concept_explained_fa_sync_${Date.now()}`;
  state._started = true;
  state.stopRequested = false;
  state.stoppedEarly = false;
...
export function requestStopConceptExplainedFaSyncAll(): ConceptExplainedFaSyncAllStatus {
  const state = getState();
