...
type State = OtherMeaningsFaSyncAllStatus & { _started: boolean };
...
function getState(): State {
  const g = globalThis as unknown as { __otherMeaningsFaSyncAll?: State };
  if (!g.__otherMeaningsFaSyncAll) {
  ...
  return g.__otherMeaningsFaSyncAll;
}

export function getOtherMeaningsFaSyncAllStatus(): OtherMeaningsFaSyncAllStatus {
  const s = getState();
...
function indexLatestAudioForOtherMeaningsFa(): Map<string, ExistingFileInfo> {
  ...
  const reNew = new RegExp(`^(?<anki>.+?)${sep}other_meanings_fa${sep}(?<ts>\\d{8,})\\.mp3$`);
  const reLegacy = new RegExp(`^(?<anki>.+)_other_meanings_fa_(?<ts>\\d{8,})\\.mp3$`);
...
async function updateNoteField(noteId: number, value: string, anki: ReturnType<typeof createAnkiConnectClient>) {
  const res = await anki.requestDetailed("updateNoteFields", {
    note: { id: noteId, fields: { other_meanings_fa: value } },
  });
...
async function runJob(state: State) {
  ...
  for (const batch of chunk(ids, 250)) {
    ...
    for (const n of infoRes.result ?? []) {
      ...
      const value = String(n.fields?.other_meanings_fa?.value ?? "");
  ...
  await runWithConcurrency(ids, concurrency, async (noteId) => {
    ...
    const newValue = generateOtherMeaningsFaValue(dbValue, ankiLinkId, audioIndex);
...
export function startOtherMeaningsFaSyncAllIfNeeded(): OtherMeaningsFaSyncAllStatus {
  const state = getState();
  if (state.running) return getOtherMeaningsFaSyncAllStatus();
  if (state._started && !state.done) return getOtherMeaningsFaSyncAllStatus();

  state.jobId = `other_meanings_fa_sync_${Date.now()}`;
  state._started = true;
  state.stopRequested = false;
  state.stoppedEarly = false;
  ...
  return getOtherMeaningsFaSyncAllStatus();
}

export function requestStopOtherMeaningsFaSyncAll(): OtherMeaningsFaSyncAllStatus {
  const state = getState();
