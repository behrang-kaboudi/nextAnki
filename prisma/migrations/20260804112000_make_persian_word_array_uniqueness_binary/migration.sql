-- Alternate spellings are distinct unless their actual string values are the
-- same; the database's accent/case-insensitive default collation must not
-- collapse different Persian or Arabic code points.
...
BEGIN
    ...
    IF (
        ...
        SELECT COUNT(DISTINCT CAST(JSON_UNQUOTE(`items`.`value`) AS BINARY))
        FROM JSON_TABLE(
            NEW.`not_normalized_texts`,
            '$[*]' COLUMNS (`value` JSON PATH '$')
        ) AS `items`
